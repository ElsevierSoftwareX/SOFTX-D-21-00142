...
%
function [sofi,grid]=sofiCumulants(stack,first,frames,region,orders,...
    gpu, display)
if nargin < 7
    display=true;
end
...
if file
   tif=Tiff(stack,'r');
   X=tif.getTag('ImageLength');        % reading tiff info(s) image dimensions
...
sofi=cell(size(grid));                  % cell array of empty matrices (size(grid))
