function [otf,psf,Nn,dxn,Nz,Nzn,dzn]=model_PSF_CF(lambda,n,NA,pixelsize,magnification,N,zrange,dz)
...
dxn = lambda/(4*NA);         % 2*Nyquist frequency in x and y. ( nyquest frequency = res/4=lamda/8*NA confocal case) 
...
res = 0.41*(lambda)/(NA);           %% lateral resolution  confocal
...
alpha=asin(NA/n);
dzn=lambda/(2*n*(1-cos(alpha)));    % Nyquist sampling in z, original 4 : lambda/(4*n*(1-cos(alpha))); 
Nz=2*ceil(zrange/dz);
dz=2*zrange/Nz;
