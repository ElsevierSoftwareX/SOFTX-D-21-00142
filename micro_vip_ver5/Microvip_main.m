...
Cell_speed=0; % cell speed inside microfluidic system um/s
shutter_speed=200;  % caracteristic of camera 
%%
frame_rate_hz=100; % camera frame rate in hz (images/sec)
%%
microscope=3;
%%
...
if  LS_ON_OFF==0 
LSW=0;   
[img,VT,psf,Nz]=Micro_img_simulation(ADN,lambda,n,NA,pixelsize,magnification,N,zrange,dz,tau,nphot,Var_GN,Mean_GN,Cell_speed,shutter_speed,microscope,frame_rate_hz,LSW);
elseif LS_ON_OFF==1 
LSW=LSWidth;    
[img,VT,psf,Nz]=Micro_img_simulation(ADN,lambda,n,NA,pixelsize,magnification,N,zrange,dz,tau,nphot,Var_GN,Mean_GN,Cell_speed,shutter_speed,microscope,frame_rate_hz,LSW);
end 
...
figure;
imshow(sum(img,3),[]); colormap magma 
title('Simulated image:XY plane projection');
figure;
imshow((squeeze(sum(img,1))),[]); colormap magma 
title('Simulated image: YZ plane projection');

% figure;
% imshow(psf(:,:,5),[]); colormap magma
% figure;
% imshow((squeeze(sum(psf,2))),[]); colormap magma
