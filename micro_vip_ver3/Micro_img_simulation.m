...
% %
% fluo_z = ADN(:, 3); % Z coordinates (nm)
% fluo_coords = ADN(:, 1:2); % X and Y coordinates (nm)
...
% fluo_frame = ceil((fluo_z + zrange) / dzn); 
% n_frames = length(-zrange:dzn:zrange); % number of images in the stack
% in_image = all(fluo_pixels > 0, 2)  & all(fluo_pixels <= N, 2) & (fluo_frame > 0) & (fluo_frame <= n_frames); 
% fluo_coords = fluo_pixels(in_image,:); % keep only fluorophores whose coordinates are inside the image
